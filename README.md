...
If a allow list is defined for a tag, it will be validated to ensure that each item in tags array is allowed.

```ruby
  class User < ActiveRecord::Base
    taggable_array :colors, allow_list: %w[red blue green]
  end
```
